...
impl NfsCache {
    ...
    pub async fn with_config(
        ...
        let disk = sled::open(disk_path.as_ref())
            .map_err(|e| crate::error::Error::InvalidOperation(format!("Failed to open disk cache: {}", e)))?;
    ...
    pub async fn get(&self, key: &str) -> Result<Option<Vec<u8>>> {
        ...
        if let Some(value) = self.disk.get(key)
            .map_err(|e| crate::error::Error::InvalidOperation(format!("Disk cache read error: {}", e)))? 
        {
            tracing::debug!("Cache HIT (disk): {}", key);
            let bytes = value.to_vec();
            
            // Promote to memory cache
            self.memory.insert(key.to_string(), Arc::new(bytes.clone())).await;
    ...
    pub async fn insert(&self, key: String, value: Vec<u8>) -> Result<()> {
        ...
        self.memory.insert(key.clone(), Arc::new(value.clone())).await;
        
        // Insert into disk cache
        self.disk.insert(key.as_bytes(), value.as_slice())
            .map_err(|e| crate::error::Error::InvalidOperation(format!("Disk cache write error: {}", e)))?;
        
        // Flush to ensure test visibility (sled buffers writes)
        self.disk.flush()
            .map_err(|e| crate::error::Error::InvalidOperation(format!("Disk cache flush error: {}", e)))?;
    ...
    pub async fn remove(&self, key: &str) -> Result<()> {
        ...
        self.disk.remove(key.as_bytes())
            .map_err(|e| crate::error::Error::InvalidOperation(format!("Disk cache remove error: {}", e)))?;
        Ok(())
    }
    ...
    pub async fn clear(&self) -> Result<()> {
        ...
        self.disk.clear()
            .map_err(|e| crate::error::Error::InvalidOperation(format!("Disk cache clear error: {}", e)))?;
        Ok(())
    }
    ...
    pub fn disk_size_bytes(&self) -> Result<u64> {
        Ok(self.disk.size_on_disk()
            .map_err(|e| crate::error::Error::InvalidOperation(format!("Failed to get disk size: {}", e)))?)
    }
    ...
    async fn maybe_evict_disk(&self) -> Result<()> {
        ...
        if size > self.max_disk_size {
            tracing::info!("Disk cache size ({} bytes) exceeds limit ({} bytes), clearing", 
                          size, self.max_disk_size);
            self.disk.clear()
                .map_err(|e| crate::error::Error::InvalidOperation(format!("Failed to clear disk cache: {}", e)))?;
        }
...
mod tests {
    ...
    async fn test_large_values() {
        ...
        cache.insert("large_key".to_string(), large_value.clone()).await.unwrap();
    ...
    async fn test_custom_cache_config() {
        ...
        let cache = NfsCache::with_config(
            temp_dir.path().join("cache"),
            config,
        ).await.unwrap();
        
        cache.insert("test".to_string(), b"value".to_vec()).await.unwrap();
