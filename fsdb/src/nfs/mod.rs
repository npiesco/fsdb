...
impl NfsServer {
    ...
    pub async fn new(db: Arc<DatabaseOps>, port: u16) -> Result<Self> {
        info!("Starting FSDB NFS server on port {} with caching enabled", port);
        ...
        match tokio::time::timeout(std::time::Duration::from_secs(5), async { ready_rx.recv().await }).await {
            Ok(Some(())) => {
            ...
            Ok(None) => {
                Err(Error::InvalidOperation("NFS server failed to start".to_string()))
            }
            Err(_) => {
                Err(Error::InvalidOperation("NFS server startup timeout".to_string()))
            }
        }
    }
    ...
    pub async fn readdir(&self, path: &str) -> Result<Vec<String>> {
        ...
        let result = fs.readdir(dirid, 0, 100).await
            .map_err(|e| Error::InvalidOperation(format!("readdir failed: {:?}", e)))?;
        
        let mut entries: Vec<String> = result.entries.iter()
    ...
    pub async fn read_file(&self, path: &str, offset: u64, size: u32) -> Result<Vec<u8>> {
        ...
        match path {
            ...
            _ if path.starts_with("/.metadata/") => {
                ...
                match filename {
                    ...
                    _ => return Err(Error::InvalidOperation(format!("Unknown metadata file: {}", path))),
                }
            }
            _ => {}
        }
        ...
        let fileid = if path == "/data/data.csv" {
            ...
            fs.refresh_parquet_files().await
                .map_err(|e| Error::InvalidOperation(format!("Failed to refresh parquet files: {:?}", e)))?;
            
            let filename = path.strip_prefix("/data/").unwrap();
            let parquet_files = fs.parquet_files.lock().await;
            let fileid_opt = parquet_files.iter()
                .find(|(_id, file_path)| {
            ...
            match fileid_opt {
                Some(id) => id,
                None => return Err(Error::InvalidOperation(format!("Parquet file not found: {}", path))),
            }
        } else {
            return Err(Error::InvalidOperation(format!("Unknown file: {}", path)));
        };
        
        let (data, _eof) = fs.read(fileid, offset, size).await
    ...
    pub async fn remove_file(&self, path: &str) -> Result<()> {
        ...
        if path == "/data/data.csv" {
            ...
            Err(Error::InvalidOperation(format!("File deletion not supported for: {}", path)))
        }
    }
    ...
    pub async fn getattr(&self, path: &str) -> Result<FileAttributes> {
        ...
        let attr = fs.getattr(fileid).await
