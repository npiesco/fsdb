...
impl WriteBuffer {
    ...
    pub async fn write(&self, data: &[u8]) -> Result<()> {
        ...
        debug!("Write buffered: {} bytes, {} rows total", data.len(), state.row_count);
        
        // Check if we need to flush
        let should_flush = self.should_flush(&state);
        
        if should_flush {
            info!("Auto-flushing buffer: {} rows, {} bytes", state.row_count, state.data.len());
    ...
    pub async fn flush(&self) -> Result<()> {
        ...
        let csv_data = {
            ...
            info!("Flushing write buffer: {} rows, {} bytes", state.row_count, state.data.len());
            state.data.clone()
        }; // Lock released here
    ...
    fn should_flush(&self, state: &BufferState) -> bool {
        ...
        if state.row_count >= self.config.max_rows {
            debug!("Row count threshold reached: {} >= {}", state.row_count, self.config.max_rows);
            return true;
        }
        
        // Flush if size exceeds threshold
        if state.data.len() >= self.config.max_bytes {
            debug!("Size threshold reached: {} >= {}", state.data.len(), self.config.max_bytes);
            return true;
        }
        ...
        if let Some(first_write) = state.first_write {
            ...
            if elapsed >= self.config.max_duration {
                debug!("Duration threshold reached: {:?} >= {:?}", elapsed, self.config.max_duration);
                return true;
            }
        }
    ...
    async fn parse_and_insert(&self, csv_data: &[u8]) -> Result<()> {
        ...
        info!("Inserting {} buffered rows into database", unified_batch.num_rows());
...
mod tests {
    ...
    async fn create_test_db() -> (DatabaseOps, TempDir) {
        ...
        let db = DatabaseOps::create(temp_dir.path().join("test_db"), schema).await.unwrap();
        (db, temp_dir)
    }
