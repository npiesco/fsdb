...
use arrow::csv::{Writer as CsvWriter, ReaderBuilder as CsvReaderBuilder};
...
impl CsvFileView {
    ...
    pub fn new_for_file(db: Arc<DatabaseOps>, file_path: String) -> Self {
        debug!("Creating CSV file view for specific file: {} (lazy loading)", file_path);
        CsvFileView {
            db,
    ...
    pub async fn apply_write(&self, data: &[u8], cached_current_csv: Option<Vec<u8>>) -> Result<()> {
        let _start_time = std::time::Instant::now();
        info!("Applying write: {} bytes", data.len());
        
        let new_csv_str = String::from_utf8_lossy(data);
        debug!("Write data preview: {}", &new_csv_str.chars().take(200).collect::<String>());
        ...
        debug!("CSV retrieval took: {:?} (cached: {})", csv_gen_duration, was_cached);
        
        let current_csv_str = String::from_utf8_lossy(&current_csv_bytes);
        
        info!("Current CSV size: {} bytes, New CSV size: {} bytes", current_csv_bytes.len(), data.len());
        
        // Check if this looks like an overwrite (has header and potentially fewer rows)
        let new_has_header = new_csv_str.lines().next().map(|l| l.contains(',') && !l.chars().next().unwrap_or('0').is_numeric()).unwrap_or(false);
        ...
        if new_has_header && data.len() < current_csv_bytes.len() {
            ...
            return self.handle_csv_overwrite(&current_csv_str, &new_csv_str).await;
    ...
    async fn handle_csv_overwrite(&self, old_csv: &str, new_csv: &str) -> Result<()> {
        ...
        let id_column_name = schema
            .fields()
            .iter()
            .find(|f| matches!(f.data_type(), arrow::datatypes::DataType::Int32 | arrow::datatypes::DataType::Int64))
            .map(|f| f.name().as_str())
            .unwrap_or_else(|| schema.field(0).name().as_str());
        
        info!("Using '{}' as ID column for row identification", id_column_name);
        ...
        info!("Old CSV has {} rows, New CSV has {} rows", old_batch.num_rows(), new_batch.num_rows());
        ...
        debug!("Built row maps: old={} rows, new={} rows", old_map.len(), new_map.len());
        ...
        info!("Changes detected: {} inserts, {} updates, {} deletes",
              insert_ids.len(), update_ids.len(), delete_ids.len());
        ...
        if !insert_ids.is_empty() || !update_ids.is_empty() {
            ...
            let source_batch = self.build_merge_source_batch(&new_batch, &insert_ids, &update_ids)?;
            ...
            if source_batch.num_rows() > 0 {
                ...
                metrics = self.db
                    ...
                    .on(format!("target.{} = source.{}", id_column_name, id_column_name))
                    .when_matched_update()
        ...
        info!("MERGE completed: {} inserts, {} updates, {} deletes in {:?} (total: {:?})",
              metrics.rows_inserted, metrics.rows_updated, delete_ids.len(),
              merge_duration, total_duration);
    ...
    fn extract_ids_from_csv(&self, csv_content: &str, id_column: &str) -> Result<HashSet<String>> {
        ...
        let id_index = headers.iter().position(|&h| h.trim() == id_column)
            .ok_or_else(|| crate::error::Error::InvalidOperation(format!("ID column '{}' not found in CSV header", id_column)))?;
    ...
    fn parse_csv_to_batch(&self, csv_content: &str) -> Result<RecordBatch> {
        ...
        if batches.len() == 1 {
            ...
            arrow::compute::concat_batches(&self.db.schema(), &batches)
                .map_err(|e| crate::error::Error::Arrow(e))
        }
    }
    
    /// Build a HashMap of ID -> Row (as string representation) for comparison
    fn build_id_to_row_map(&self, batch: &RecordBatch, id_column: &str) -> Result<HashMap<String, String>> {
        use arrow::array::*;
        ...
        let id_array = batch.column_by_name(id_column)
            .ok_or_else(|| crate::error::Error::InvalidOperation(format!("ID column '{}' not found", id_column)))?;
        ...
        for row_idx in 0..batch.num_rows() {
            ...
            let id_str = match id_array.data_type() {
                ...
                _ => {
                    return Err(crate::error::Error::InvalidOperation("Unsupported ID column type".to_string()));
                }
            };
    ...
    fn build_merge_source_batch(&self, batch: &RecordBatch, insert_ids: &[String], update_ids: &[String]) -> Result<RecordBatch> {
        use arrow::array::*;
        ...
        let id_column = schema
            ...
            .ok_or_else(|| crate::error::Error::InvalidOperation("No ID column found".to_string()))?;
        
        let id_array = batch.column_by_name(id_column.name())
            .ok_or_else(|| crate::error::Error::InvalidOperation("ID column not found in batch".to_string()))?;
        ...
        debug!("Filtering batch to {} rows for MERGE", indices_to_include.len());
        ...
        for col_idx in 0..batch.num_columns() {
            ...
            let filtered_col = compute::take(col, &indices_array, None)
                .map_err(|e| crate::error::Error::Arrow(e))?;
        ...
        debug!("Built MERGE source batch with {} rows", indices_to_include.len());
...
impl QueryFile {
    ...
    pub async fn write_query(&mut self, query_bytes: &[u8]) -> Result<()> {
        ...
        info!("Query executed, generated {} bytes of CSV results", buffer.len());
    ...
    /// Read query results
    pub async fn read(&mut self, offset: u64, count: u64) -> Result<Vec<u8>> {
        let content = self.result_content.as_ref()
            .ok_or_else(|| crate::error::Error::InvalidOperation("No query has been executed".to_string()))?;
...
impl StatsFile {
    ...
    pub async fn generate_content(&self) -> Result<Vec<u8>> {
        ...
        let total_rows = match self.db.query("SELECT COUNT(*) as count FROM data").await {
            Ok(batches) => {
                if !batches.is_empty() {
                    if let Some(array) = batches[0].column(0).as_any().downcast_ref::<arrow::array::Int64Array>() {
