...
pub fn parse_s3_uri(s3_path: &str) -> Result<(String, String)> {
    let url = Url::parse(s3_path)
        .map_err(|e| crate::Error::Other(format!("Invalid S3 URI: {}", e)))?;
    
    if url.scheme() != "s3" {
        return Err(crate::Error::Other(format!("Invalid S3 URI scheme: {}", url.scheme())));
    }
...
pub fn parse_s3_url(s3_path: &str) -> Result<Url> {
    Url::parse(s3_path)
        .map_err(|e| crate::Error::Other(format!("Invalid S3 URL: {}", e)))
}
