...
impl MergeBuilder {
    ...
    pub async fn execute(self) -> Result<MergeMetrics> {
        ...
        info!("Clauses: {} updates, {} deletes, {} inserts", 
              self.matched_updates.len(), self.matched_deletes.len(), 
              self.not_matched_inserts.len());
        
        // Validate inputs
        let (source_data, source_alias) = self.source
            .as_ref()
            .ok_or_else(|| Error::Other("Source data not provided for MERGE".to_string()))?;
        
        let join_condition = self.join_condition
            .as_ref()
        ...
        for delete_clause in &self.matched_deletes {
            let (ids, count) = self.collect_matched_delete_ids(
                &ctx,
                source_alias,
                join_condition,
                delete_clause
        ...
        for update_clause in &self.matched_updates {
            let (ids, batches, count) = self.collect_matched_update_data(
                &ctx,
                source_alias,
                join_condition,
                update_clause,
        ...
        for insert_clause in &self.not_matched_inserts {
            let (batches, count) = self.collect_not_matched_insert_data(
                &ctx,
                source_alias,
                join_condition,
                insert_clause,
        ...
        if !all_ids_to_delete.is_empty() {
            debug!("Executing batched DELETE for {} total rows", all_ids_to_delete.len());
            // Deduplicate IDs
            all_ids_to_delete.sort_unstable();
            all_ids_to_delete.dedup();
            
            let id_list = all_ids_to_delete.iter().map(|id| id.to_string()).collect::<Vec<_>>().join(", ");
        ...
        if !all_batches_to_insert.is_empty() {
            debug!("Executing batched INSERT for {} batches in single operation", all_batches_to_insert.len());
        ...
        info!("MERGE completed: inserted={}, updated={}, deleted={}", 
    ...
    async fn collect_matched_delete_ids(
        ...
        let batches = df.collect().await
            .map_err(|e| Error::Other(format!("Failed to collect MATCHED DELETE results: {}", e)))?;
    ...
    async fn collect_matched_update_data(
        ...
        let update_columns = if clause.updates.is_empty() {
            ...
            source_data.schema().fields().iter()
                .filter(|f| f.name() != "_op")
        ...
        let select_list = update_columns.iter()
        ...
        let batches = df.collect().await
            .map_err(|e| Error::Other(format!("Failed to collect MATCHED UPDATE results: {}", e)))?;
        ...
        debug!("Collected {} rows for update (IDs to delete + batches to insert)", row_count);
    ...
    async fn collect_not_matched_insert_data(
        ...
        let select_list = insert_columns.iter()
            .map(|(col, expr)| format!("{} as {}", expr, col))
            .collect::<Vec<_>>()
            .join(", ");
        ...
        // Execute query
        let df = ctx.sql(&sql).await
            .map_err(|e| Error::Other(format!("Failed to execute NOT MATCHED INSERT query: {}", e)))?;
        
        let batches = df.collect().await
            .map_err(|e| Error::Other(format!("Failed to collect NOT MATCHED INSERT results: {}", e)))?;
    ...
    fn extract_ids_from_batches(&self, batches: &[RecordBatch]) -> Result<Vec<i64>> {
        ...
        for batch in batches {
            ...
            let id_column = schema.fields().iter()
            ...
            match id_array.data_type() {
                DataType::Int32 => {
                    let int_array = id_array.as_any().downcast_ref::<Int32Array>()
                ...
                DataType::Int64 => {
                    let int_array = id_array.as_any().downcast_ref::<Int64Array>()
