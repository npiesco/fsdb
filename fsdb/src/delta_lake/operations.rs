...
pub async fn optimize_table(
    ...
    let (_table, delta_metrics) = optimize_builder.await
        .map_err(|e| Error::DeltaTable(e))?;
...
pub async fn vacuum_table(
    ...
    // Execute VACUUM
    let (table_result, metrics) = vacuum_builder.await
        .map_err(|e| Error::DeltaTable(e))?;
...
pub async fn vacuum_dry_run(
    ...
    // Execute dry run
    let (_table_result, metrics) = vacuum_builder.await
        .map_err(|e| Error::DeltaTable(e))?;
    
    // Return list of files that would be deleted
    let files_to_delete: Vec<String> = metrics.files_deleted
        .iter()
        .map(|path| path.to_string())
        .collect();
    
    info!("VACUUM dry run: {} files would be deleted", files_to_delete.len());
...
pub async fn zorder_table(
    ...
    let optimize_builder = DeltaOps(table).optimize()
