...
impl QueryExecutor {
    ...
    pub async fn register_table(&self, table_name: &str, schema: SchemaRef, data: Vec<RecordBatch>) -> Result<()> {
        // Lock to prevent concurrent registration of the same table
        let _lock = self.registration_lock.lock().await;
        
        debug!("Registering table '{}' with {} batches", table_name, data.len());
...
mod tests {
    ...
    fn create_test_batch(schema: SchemaRef) -> RecordBatch {
        ...
        let dept_array = Arc::new(StringArray::from(vec!["Engineering", "Sales", "Engineering", "Marketing", "Sales"])) as ArrayRef;
        let salary_array = Arc::new(Float64Array::from(vec![Some(120000.0), Some(85000.0), Some(95000.0), Some(78000.0), Some(92000.0)])) as ArrayRef;
        
        RecordBatch::try_new(
            schema,
            vec![id_array, name_array, dept_array, salary_array],
        ).unwrap()
    }
    ...
    async fn test_execute_simple_select() {
        ...
        executor.register_table("employees", schema, vec![batch]).await.unwrap();
        
        // Simple SELECT *
        let results = executor.execute_sql("SELECT * FROM employees").await.unwrap();
    ...
    async fn test_execute_select_with_projection() {
        ...
        let results = executor.execute_sql("SELECT name, department FROM employees").await.unwrap();
    ...
    async fn test_execute_aggregation() {
        ...
        let results = executor.execute_sql(
            "SELECT COUNT(*) as count, AVG(salary) as avg_salary FROM employees"
        ).await.unwrap();
    ...
    async fn test_execute_join() {
        ...
        executor.register_table("employees", emp_schema, vec![emp_batch]).await.unwrap();
        executor.register_table("departments", dept_schema, vec![dept_batch]).await.unwrap();
    ...
    async fn test_execute_multiple_batches() {
        ...
        let dept_array2 = Arc::new(StringArray::from(vec!["Engineering", "Sales", "Marketing"])) as ArrayRef;
        let salary_array2 = Arc::new(Float64Array::from(vec![Some(88000.0), Some(91000.0), Some(79000.0)])) as ArrayRef;
        ...
        executor.register_table("employees", schema, vec![batch1, batch2]).await.unwrap();
        
        // Query should return all rows from both batches
        let count = executor.execute_sql_count("SELECT * FROM employees").await.unwrap();
