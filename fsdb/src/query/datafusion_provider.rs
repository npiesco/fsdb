...
impl FsdbTableProvider {
    ...
    pub fn new(schema: SchemaRef, data: Vec<RecordBatch>) -> Result<Self> {
        ...
        Ok(Self { inner: Arc::new(mem_table) })
    }
    ...
    pub fn from_parquet_files(
        ...
        debug!("Loading {} parquet files into DataFusion table provider", file_paths.len());
        ...
        debug!("Loaded {} total rows from parquet files", combined_batch.num_rows());
...
mod tests {
    ...
    fn create_test_batch(schema: SchemaRef) -> RecordBatch {
        ...
        let name_array = Arc::new(StringArray::from(vec!["Alice", "Bob", "Charlie", "David", "Eve"])) as ArrayRef;
        let age_array = Arc::new(Int32Array::from(vec![Some(25), Some(30), None, Some(35), Some(28)])) as ArrayRef;
        let score_array = Arc::new(Float64Array::from(vec![Some(95.5), Some(87.3), Some(92.1), None, Some(88.9)])) as ArrayRef;
        
        RecordBatch::try_new(
            schema,
            vec![id_array, name_array, age_array, score_array],
        ).unwrap()
    }
    ...
    async fn test_table_provider_where_clause() {
        ...
        let df = ctx.sql("SELECT name, age FROM test_table WHERE age > 28").await.unwrap();
        let results = df.collect().await.unwrap();
    ...
    async fn test_table_provider_aggregation() {
        ...
        // Aggregation query
        let df = ctx.sql("SELECT COUNT(*) as count, AVG(age) as avg_age FROM test_table").await.unwrap();
        let results = df.collect().await.unwrap();
    ...
    async fn test_table_provider_order_by() {
        ...
        // ORDER BY query
        let df = ctx.sql("SELECT name FROM test_table ORDER BY name DESC").await.unwrap();
        let results = df.collect().await.unwrap();
    ...
    async fn test_table_provider_multiple_batches() {
        ...
        let score_array2 = Arc::new(Float64Array::from(vec![Some(91.2), Some(85.7), Some(94.3)])) as ArrayRef;
    ...
    async fn test_table_provider_null_handling() {
        ...
        // Query with IS NULL
        let df = ctx.sql("SELECT name FROM test_table WHERE age IS NULL").await.unwrap();
        let results = df.collect().await.unwrap();
    ...
    async fn test_table_provider_join() {
        ...
        ctx.register_table("departments", Arc::new(provider2)).unwrap();
        
        // JOIN query
        let df = ctx.sql("SELECT u.name, d.department FROM users u JOIN departments d ON u.id = d.user_id").await.unwrap();
        let results = df.collect().await.unwrap();
    ...
    async fn test_from_parquet_files_integration() {
        ...
        let ctx = SessionContext::new();
        ctx.register_table("parquet_data", Arc::new(provider)).unwrap();
        
        // Query all data
        let df = ctx.sql("SELECT * FROM parquet_data ORDER BY id").await.unwrap();
        let results = df.collect().await.unwrap();
        
        // Should have combined data from both files
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].num_rows(), 5, "Should have 5 total rows from both files");
        ...
        // Query with WHERE clause
        let df = ctx.sql("SELECT name, score FROM parquet_data WHERE score > 90").await.unwrap();
        let results = df.collect().await.unwrap();
        
        assert_eq!(results[0].num_rows(), 3, "Should have 3 rows with score > 90");
        
        // Query with aggregation
        let df = ctx.sql("SELECT COUNT(*) as count, AVG(score) as avg_score FROM parquet_data").await.unwrap();
        let results = df.collect().await.unwrap();
