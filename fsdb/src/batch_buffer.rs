...
impl BatchBuffer {
    ...
    pub async fn push(&self, batch: RecordBatch) -> bool {
        ...
        debug!("Buffer now has {} batches with {} total rows", state.batches.len(), total_rows);
    ...
    pub fn concatenate_batches(schema: &SchemaRef, batches: Vec<RecordBatch>) -> crate::Result<RecordBatch> {
        if batches.is_empty() {
            return Err(crate::Error::InvalidOperation("Cannot concatenate empty batch list".into()));
        }
        ...
        info!("Concatenated {} batches into single batch with {} rows", 
              batches.len(), concatenated.num_rows());
