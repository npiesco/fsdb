...
use fsdb::{DatabaseOps, error::Result};
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Mount { db_path, mount_point, port } => {
            // Open database
            ...
            let mount_status = std::process::Command::new("mount")
                ...
                .arg("anon")
                .arg(format!("\\\\localhost\\"))
                .arg(format!("{}:", mount_point.to_string_lossy().chars().next().unwrap()))
                .status();
            ...
            match mount_status {
                ...
                Ok(s) => {
                    ...
                    eprintln!("");
                    eprintln!("NFS server is running on port {}. You can mount manually:", port);
                    #[cfg(target_os = "macos")]
        ...
        Commands::S3Test { s3_path, endpoint, access_key, secret_key } => {
            ...
            let db2 = DatabaseOps::open_with_s3(
                &s3_path,
                &endpoint,
                &access_key,
                &secret_key,
