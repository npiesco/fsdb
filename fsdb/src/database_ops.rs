...
use crate::delta_lake::stats::{ColumnStats, get_column_statistics_from_delta};
...
impl DatabaseOps {
    ...
    pub async fn create_with_delta_native<P: AsRef<Path>>(path: P, schema: SchemaRef) -> Result<Self> {
        let base_path = path.as_ref().to_path_buf();
        info!("Creating database in Delta Lake native mode at: {}", base_path.display());
        ...
        let _delta_table = {
            ...
            use deltalake::kernel::{StructType, StructField};
            use url::Url;
            ...
            let ops = DeltaOps::try_from_uri(table_url).await
                .map_err(|e| Error::DeltaTable(e))?;
    ...
    pub async fn open_delta_native<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let (_delta_table, schema) = {
            ...
            let arrow_fields: Result<Vec<_>> = delta_schema.fields().map(|field| {
    ...
    pub async fn create_with_s3(
        ...
        use deltalake::DeltaOps;
        use deltalake::kernel::{StructType, StructField};
        use crate::storage::s3::{create_delta_storage_options, parse_s3_url, get_s3_cache_path};
        ...
        let ops = DeltaOps::try_from_uri_with_storage_options(s3_url.clone(), storage_options.clone()).await
            .map_err(|e| Error::DeltaTable(e))?;
    ...
    pub async fn open_with_s3(
        ...
        let s3_url = parse_s3_url(s3_path)?;
        let table = open_table_with_storage_options(s3_url, storage_options.clone()).await
            .map_err(|e| Error::DeltaTable(e))?;
        ...
        info!("Opened Delta Lake from S3 with {} fields", schema.fields().len());
    ...
    fn arrow_to_delta_type(arrow_type: &arrow::datatypes::DataType) -> Result<deltalake::kernel::DataType> {
        ...
        match arrow_type {
            ...
            other => Err(Error::Other(format!("Unsupported Arrow type for Delta Lake: {:?}", other))),
        }
    }
    
    /// Helper: Convert Delta Lake DataType to Arrow DataType
    fn delta_to_arrow_type(delta_type: &deltalake::kernel::DataType) -> Result<arrow::datatypes::DataType> {
        ...
        match delta_type {
            DeltaDataType::Primitive(prim) => match prim {
                ...
                deltalake::kernel::PrimitiveType::TimestampNtz => Ok(ArrowDataType::Timestamp(TimeUnit::Microsecond, None)),
                other => Err(Error::Other(format!("Unsupported Delta Lake primitive type: {:?}", other))),
            },
            other => Err(Error::Other(format!("Unsupported Delta Lake type: {:?}", other))),
        }
    }
    
    /// Create a new database with authentication enabled
    pub async fn create_with_auth<P: AsRef<Path>>(path: P, schema: SchemaRef, auth_enabled: bool) -> Result<Self> {
    ...
    pub async fn open_with_credentials<P: AsRef<Path>>(path: P, credentials: crate::security::Credentials) -> Result<Self> {
        let base_path = path.as_ref().to_path_buf();
    ...
    /// Create a new database with S3/MinIO backend
    ...
    pub async fn create_user(&self, username: &str, password: &str, roles: &[&str]) -> Result<()> {
        ...
        match &result {
            Ok(_) => {
                self.audit_log("CREATE_USER", &format!("username={}, roles={:?}", username, roles), true).await;
            }
            Err(e) => {
                self.audit_log("CREATE_USER", &format!("username={}: {}", username, e), false).await;
            }
        }
    ...
    pub async fn insert(&self, batch: RecordBatch) -> Result<u64> {
        ...
        match &result {
            Ok(_) => {
                ...
                self.audit_log("INSERT", &format!("{} rows", num_rows), true).await;
            }
            Err(e) => {
                self.metrics.total_errors.fetch_add(1, Ordering::Relaxed);
                self.audit_log("INSERT", &format!("failed: {}", e), false).await;
            }
        }
        result
    }
    ...
    pub async fn insert_buffered(&self, batch: RecordBatch) -> Result<()> {
        ...
        if should_flush {
            ...
            info!("Auto-flushing batch buffer: {} rows across {} batches", 
    ...
    pub async fn flush_write_buffer(&self) -> Result<()> {
        ...
        let (batch_count, total_rows) = self.batch_buffer.stats().await;
        info!("Flushing batch buffer: {} rows across {} batches", 
              total_rows, batch_count);
    ...
    async fn flush_batches(&self, batches: Vec<RecordBatch>) -> Result<()> {
        ...
        if batches.len() == 1 {
            return self.insert(batches.into_iter().next().unwrap()).await.map(|_| ());
        }
        
        // Multiple batches: concatenate them into one batch for single transaction
        let concatenated = crate::batch_buffer::BatchBuffer::concatenate_batches(&self.schema, batches)?;
    ...
    pub async fn get_delta_table(&self) -> Result<deltalake::DeltaTable> {
        ...
        let table = if let (Some(s3_url), Some(storage_options)) = (&self.s3_url, &self.s3_storage_options) {
            // S3 backend
            let s3_url_parsed = parse_s3_url(s3_url)?;
            open_table_with_storage_options(s3_url_parsed, storage_options.clone()).await
                .map_err(|e| Error::DeltaTable(e))?
        } else {
            // Local backend
    ...
    async fn query_delta_native(&self, sql: &str) -> Result<Vec<RecordBatch>> {
        use deltalake::{open_table, open_table_with_storage_options, datafusion::prelude::SessionContext};
        use url::Url;
        use crate::storage::s3::parse_s3_url;
    ...
    async fn delete_delta_native(&self, where_clause: &str) -> Result<usize> {
        ...
        info!("Successfully deleted {} rows from Delta Lake", deleted_count);
        Ok(deleted_count)
    }
    ...
    pub(crate) fn unify_batch_schema(&self, batch: &RecordBatch, unified_schema: &SchemaRef) -> Result<RecordBatch> {
        ...
        for unified_field in unified_schema.fields() {
            ...
            if let Some((idx, _batch_field)) = batch.schema().fields().iter().enumerate().find(|(_i, f)| f.name() == field_name) {
    ...
    pub async fn query(&self, sql: &str) -> Result<Vec<RecordBatch>> {
        ...
        match &result {
            ...
            Err(e) => {
                self.metrics.total_errors.fetch_add(1, Ordering::Relaxed);
                self.audit_log("SELECT", &format!("{}: {}", sql, e), false).await;
            }
        }
        result
    }
    ...
    async fn query_inner(&self, sql: &str) -> Result<Vec<RecordBatch>> {
        ...
        for file_stat in &file_stats {
            ...
            for (column, operator, value) in &predicates {
                if crate::delta_lake::data_skipping::can_skip_file(file_stat, column, operator, value) {
                    can_skip = true;
                    break;
                }
            }
        ...
        info!("Data skipping: {} files total, {} to read, {} skipped", 
            file_stats.len(), files_to_read.len(), files_to_skip.len());
    ...
    pub async fn query_version(&self, sql: &str, version: i64) -> Result<Vec<RecordBatch>> {
        info!("Executing time travel query at version {}: {}", version, sql);
        ...
        match &result {
            Ok(_) => {
                ...
                self.audit_log("SELECT_VERSION", &format!("v{}: {}", version, sql), true).await;
            }
            Err(e) => {
                self.metrics.total_errors.fetch_add(1, Ordering::Relaxed);
                self.audit_log("SELECT_VERSION", &format!("v{}: {}: {}", version, sql, e), false).await;
            }
        }
        result
    }
    ...
    async fn query_version_inner(&self, sql: &str, version: i64) -> Result<Vec<RecordBatch>> {
        ...
        info!("Querying Delta Lake with SQL at version {}: {}", version, sql);
        ...
        let table = if let Some(ref s3_url) = self.s3_url {
            if let Some(ref storage_options) = self.s3_storage_options {
                ...
                builder.load().await.map_err(|e| {
                    error!("Failed to load Delta table at version {} from S3: {}", version, e);
                    Error::DeltaTable(e)
                })?
            } else {
                return Err(Error::Other("S3 storage options not configured".to_string()));
            }
        } else {
            let table_path = self.base_path.to_str()
        ...
        info!("Time travel query at version {} returned {} batches", version, batches.len());
        Ok(batches)
    }
    ...
    pub async fn query_timestamp(&self, sql: &str, timestamp_ms: i64) -> Result<Vec<RecordBatch>> {
        info!("Executing time travel query at timestamp {}: {}", timestamp_ms, sql);
        ...
        match &result {
            Ok(_) => {
                ...
                self.audit_log("SELECT_TIMESTAMP", &format!("@{}: {}", timestamp_ms, sql), true).await;
            }
            Err(e) => {
                self.metrics.total_errors.fetch_add(1, Ordering::Relaxed);
                self.audit_log("SELECT_TIMESTAMP", &format!("@{}: {}: {}", timestamp_ms, sql, e), false).await;
            }
        }
        result
    }
    ...
    async fn query_timestamp_inner(&self, sql: &str, timestamp_ms: i64) -> Result<Vec<RecordBatch>> {
        use tracing::error;
        use deltalake::datafusion::prelude::SessionContext;
        use chrono::{DateTime, Utc};
        use url::Url;
        
        info!("Querying Delta Lake with SQL at timestamp {}: {}", timestamp_ms, sql);
        ...
        let table = if let Some(ref s3_url) = self.s3_url {
            if let Some(ref storage_options) = self.s3_storage_options {
                ...
                builder.load().await.map_err(|e| {
                    error!("Failed to load Delta table at timestamp {} from S3: {}", timestamp_ms, e);
                    Error::DeltaTable(e)
                })?
            } else {
                return Err(Error::Other("S3 storage options not configured".to_string()));
            ...
            deltalake::DeltaTableBuilder::from_uri(url)
                ...
                .await
                .map_err(|e| {
                    error!("Failed to load Delta table at timestamp {}: {}", timestamp_ms, e);
                    Error::DeltaTable(e)
                })?
        };
        ...
        info!("Time travel query at timestamp {} returned {} batches", timestamp_ms, batches.len());
        Ok(batches)
    }
    ...
    pub async fn query_file(&self, file_path: &str) -> Result<Vec<RecordBatch>> {
        ...
        if !full_path.exists() {
            return Err(Error::RecordNotFound(format!("File not found: {}", file_path)));
        }
    ...
    pub async fn delete(&self, _file_path: &str) -> Result<u64> {
        ...
        Err(Error::Other("Use delete_rows_where() for row-level deletion in Delta Lake".to_string()))
    }
    ...
    pub async fn delete_rows_where(&self, where_clause: &str) -> Result<usize> {
        ...
        match &result {
            Ok(count) => {
                self.metrics.total_deletes.fetch_add(1, Ordering::Relaxed);
                self.audit_log("DELETE", &format!("WHERE {} ({} rows)", where_clause, count), true).await;
            }
            Err(e) => {
                self.metrics.total_errors.fetch_add(1, Ordering::Relaxed);
                self.audit_log("DELETE", &format!("WHERE {}: {}", where_clause, e), false).await;
            }
        }
        result
    }
    ...
    /// Get deletion vector statistics
    ...
    pub async fn optimize(&self) -> Result<()> {
        ...
        match &result {
            Ok(metrics) => {
                info!("OPTIMIZE completed: {} files added, {} files removed", 
                    metrics.num_files_added, metrics.num_files_removed);
                self.audit_log("OPTIMIZE", &format!("compacted {} -> {} files", 
                    metrics.num_files_removed, metrics.num_files_added), true).await;
            }
            Err(e) => {
                self.metrics.total_errors.fetch_add(1, Ordering::Relaxed);
                self.audit_log("OPTIMIZE", &format!("failed: {}", e), false).await;
            }
        }
        result.map(|_| ())
    }
    ...
    pub async fn optimize_with_filter(&self, filter: &str) -> Result<()> {
        ...
        match &result {
            Ok(metrics) => {
                info!("OPTIMIZE with filter completed: {} files added, {} files removed", 
                    metrics.num_files_added, metrics.num_files_removed);
                self.audit_log("OPTIMIZE", &format!("filter='{}': {} -> {} files", 
                    filter, metrics.num_files_removed, metrics.num_files_added), true).await;
            }
            Err(e) => {
                self.metrics.total_errors.fetch_add(1, Ordering::Relaxed);
                self.audit_log("OPTIMIZE", &format!("filter='{}': {}", filter, e), false).await;
            }
        }
        result.map(|_| ())
    }
    ...
    pub async fn optimize_with_target_size(&self, target_size_bytes: u64) -> Result<()> {
        info!("Running Delta Lake OPTIMIZE with target size: {} bytes", target_size_bytes);
        ...
        match &result {
            Ok(metrics) => {
                info!("OPTIMIZE with target size completed: {} files added, {} files removed", 
                    metrics.num_files_added, metrics.num_files_removed);
                self.audit_log("OPTIMIZE", &format!("target_size={}: {} -> {} files", 
                    target_size_bytes, metrics.num_files_removed, metrics.num_files_added), true).await;
            }
            Err(e) => {
                self.metrics.total_errors.fetch_add(1, Ordering::Relaxed);
                self.audit_log("OPTIMIZE", &format!("target_size={}: {}", target_size_bytes, e), false).await;
            }
        }
        result.map(|_| ())
    }
    ...
    async fn optimize_inner(&self, filter: Option<&str>, target_size: Option<u64>) -> Result<crate::delta_lake::OptimizeMetrics> {
    ...
    pub async fn vacuum(&self, retention_hours: u64) -> Result<()> {
        info!("Running Delta Lake VACUUM with {} hour retention", retention_hours);
        ...
        match &result {
            Ok(deleted_count) => {
                ...
                self.audit_log("VACUUM", &format!("retention={}h: {} files deleted", 
                    retention_hours, deleted_count), true).await;
            }
            Err(e) => {
                self.metrics.total_errors.fetch_add(1, Ordering::Relaxed);
                self.audit_log("VACUUM", &format!("retention={}h: {}", retention_hours, e), false).await;
            }
        }
        result.map(|_| ())
    }
    ...
    pub async fn vacuum_dry_run(&self, retention_hours: u64) -> Result<Vec<String>> {
        info!("Running Delta Lake VACUUM dry run with {} hour retention", retention_hours);
    ...
    pub async fn zorder(&self, columns: &[&str]) -> Result<()> {
        ...
        if columns.is_empty() {
            return Err(Error::InvalidOperation("Z-ORDER requires at least one column".to_string()));
        }
        
        let result = self.zorder_inner(columns).await;
        match &result {
            Ok(metrics) => {
                info!("Z-ORDER completed: {} files added, {} files removed", 
                    metrics.num_files_added, metrics.num_files_removed);
                self.audit_log("ZORDER", &format!("columns={:?}: {} -> {} files", 
                    columns, metrics.num_files_removed, metrics.num_files_added), true).await;
            }
            Err(e) => {
                self.metrics.total_errors.fetch_add(1, Ordering::Relaxed);
                self.audit_log("ZORDER", &format!("columns={:?}: {}", columns, e), false).await;
            }
        }
        result.map(|_| ())
    }
    ...
    pub async fn health_check(&self) -> HealthStatus {
        ...
        HealthStatus {
            status: if delta_log_exists { "healthy" } else { "not_initialized" }.to_string(),
            uptime_seconds: self.metrics.start_time.elapsed().as_secs_f64(),
    ...
    pub async fn backup<P: AsRef<Path>>(&self, backup_path: P) -> Result<()> {
        ...
        crate::metadata::backup::count_parquet_files(backup_path, &mut total_files, &mut total_size_bytes)?;
        
        // Query for total row count
        let total_rows = self.query("SELECT COUNT(*) as count FROM data").await
            .ok()
            .and_then(|batches| batches.first().cloned())
            .and_then(|batch| batch.column(0).as_any().downcast_ref::<arrow::array::Int64Array>()
                .filter(|arr| arr.len() > 0 && !arr.is_null(0))
        ...
        info!("Backup completed: {} files, {} rows, {} bytes", total_files, total_rows, total_size_bytes);
        Ok(())
    }
    
    /// Restore database from backup
    pub async fn restore<P: AsRef<Path>>(backup_path: P, restore_path: P) -> Result<()> {
        info!("Restoring database from {} to {}", backup_path.as_ref().display(), restore_path.as_ref().display());
    ...
    pub async fn backup_incremental<P: AsRef<Path>>(&self, base_backup_path: P, incremental_path: P) -> Result<()> {
        let base_backup_path = base_backup_path.as_ref();
        let incremental_path = incremental_path.as_ref();
        info!("Creating incremental backup at: {}", incremental_path.display());
        ...
        info!("Incremental backup completed: {} files, {} bytes", total_files, total_size_bytes);
        Ok(())
    }
    ...
    pub async fn restore_to_transaction<P: AsRef<Path>>(
        ...
        info!("Restoring to timestamp {} from {}", target_txn_id, backup_path_buf.display());
        ...
        tokio::task::spawn_blocking(move || {
            ...
            info!("Mapped timestamp {} to Delta version {}", target_txn_id, target_version);
...
mod tests {
    ...
    fn create_test_batch(schema: SchemaRef, start_id: i32) -> RecordBatch {
        let id_array = Arc::new(Int32Array::from(vec![start_id, start_id + 1, start_id + 2])) as ArrayRef;
    ...
    async fn test_database_create_and_insert() {
        ...
        let db = DatabaseOps::create(temp_dir.path(), schema.clone()).await.unwrap();
    ...
    async fn test_database_insert_and_query_with_filter() {
        ...
        let results = db.query("SELECT name, email FROM data WHERE id > 1").await.unwrap();
    ...
    async fn test_database_multiple_inserts() {
        ...
        assert!(txn2 > txn1, "Transaction ID 2 should be greater than transaction ID 1");
        
        // Query all data
        let results = db.query("SELECT COUNT(*) as count FROM data").await.unwrap();
        assert_eq!(results.len(), 1);
        
        let count_col = results[0].column(0).as_any().downcast_ref::<arrow::array::Int64Array>().unwrap();
    ...
    async fn test_database_aggregation_query() {
        ...
        let results = db.query(
            "SELECT COUNT(*) as total, COUNT(DISTINCT name) as unique_names FROM data"
        ).await.unwrap();
        
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].num_rows(), 1);
        
        let total_col = results[0].column(0).as_any().downcast_ref::<arrow::array::Int64Array>().unwrap();
    ...
    async fn test_database_delta_log_written() {
        ...
        assert!(delta_log_dir.exists(), "Delta Lake transaction log directory should exist");
        
            let delta_files: Vec<_> = std::fs::read_dir(&delta_log_dir).unwrap()
                .filter_map(|e| e.ok())
                .filter(|e| e.path().extension().and_then(|s| s.to_str()) == Some("json"))
                .collect();
        assert!(delta_files.len() > 0, "Delta Lake log should have at least one commit file");
    }
}
