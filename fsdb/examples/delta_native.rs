...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  ✓ Delta Lake _delta_log directory exists: {}", db_path.join("_delta_log").exists());
    println!();
    ...
    for batch in &results {
        ...
        let salaries = batch.column(3).as_any().downcast_ref::<Float64Array>().unwrap();
        
        for i in 0..batch.num_rows() {
            println!("  ID: {}, Name: {}, Department: {}, Salary: ${:.2}", 
                ids.value(i),
                names.value(i),
                depts.value(i),
                salaries.value(i)
    ...
    let eng_results = db.query("SELECT name, salary FROM data WHERE department = 'Engineering' ORDER BY salary DESC").await?;
    ...
    let count = after_delete[0].column(0).as_any().downcast_ref::<arrow::array::Int64Array>().unwrap().value(0);
    ...
    let evolved_results = db.query("SELECT name, hire_date FROM data WHERE hire_date IS NOT NULL").await?;
    println!("Employees with hire dates:");
    for batch in &evolved_results {
        let names = batch.column(0).as_any().downcast_ref::<StringArray>().unwrap();
        let dates = batch.column(1).as_any().downcast_ref::<StringArray>().unwrap();
    ...
    let txn_count = txn_results[0].column(0).as_any().downcast_ref::<arrow::array::Int64Array>().unwrap().value(0);
    ...
    let final_results = db_reopened.query("SELECT COUNT(*) as count FROM data").await?;
    let final_count = final_results[0].column(0).as_any().downcast_ref::<arrow::array::Int64Array>().unwrap().value(0);
    ...
    println!("  ✓ Delta Lake _delta_log directory: {}", delta_log_dir.exists());
