...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Filtered query
    let electronics = db.query("SELECT product_name, price FROM data WHERE category = 'Electronics'").await?;
    ...
    println!("   ✓ Average price calculated: {} batches returned", avg_price_results.len());
    ...
    let db_reopened = DatabaseOps::open_with_s3(
        s3_path,
        &endpoint,
        &access_key,
        &secret_key,
    ...
    let results_after_reopen = db_reopened.query("SELECT COUNT(*) as total FROM data").await?;
    println!("   ✓ Verified data persistence in S3");
    println!("   ✓ All {} rows recovered from S3 (query returned {} batches)", row_count, results_after_reopen.len());
