...
use fsdb::metadata::{SchemaManager, Schema as FsdbSchema, SchemaField};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let count = count_result[0].column(0).as_any().downcast_ref::<arrow::array::Int64Array>().unwrap().value(0);
    println!("   ✓ Total rows across all schema versions: {}", count);
    
    // Query specific columns - old rows will have NULL for 'age'
    let results = db.query("SELECT id, name, age FROM data ORDER BY id").await?;
    ...
    for batch in &results {
        ...
        for i in 0..batch.num_rows() {
            ...
            println!("     Row {}: id={}, name={}, age={}", i+1, id, name, age_str);
        }
    }
    ...
    for version in history {
        println!("     v{}: {} fields", version.version, version.schema.fields.len());
    }
