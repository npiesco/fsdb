<div align="center">
  ...
  <p><em>A columnar database engine where UNIX tools (cat, grep, awk, sed) trigger Delta Lake operations including MERGE (UPSERT), ACID transactions, and native format storage. Works with local filesystem directories and object storage/S3. Built on Apache Arrow, Parquet, and DataFusion for high-performance analytics workloads.</em></p>
...
- **Query with UNIX Tools**: `cat`, `grep`, `awk`, `wc`, `head`, `tail` all work on mounted database
- **Write with Standard Commands**: `echo >>`, `sed -i`, `vim` - all writes persist to Delta Lake
