...
use arrow::array::{Array, ArrayRef, Int32Array, StringArray, RecordBatch};
...
async fn test_query_with_schema_evolution() {
    ...
    assert_eq!(batch.num_columns(), 3, "Unified schema should have 3 columns");
    ...
    let age_array = batch.column(2).as_any().downcast_ref::<Int32Array>().unwrap();
...
async fn test_query_projection_with_evolved_schema() {
    ...
    let results = db.query("SELECT id, name FROM data ORDER BY id").await
    ...
    let results = db.query("SELECT id, department FROM data ORDER BY id").await
        .expect("Query with new column should work");
    
    let dept_array = results[0].column(1).as_any().downcast_ref::<StringArray>().unwrap();
    assert!(dept_array.is_null(0), "Alice should have NULL department");
    assert_eq!(dept_array.value(1), "Engineering", "Bob should have Engineering dept");
...
async fn test_schema_compatibility() {
    ...
    let db = DatabaseOps::create(db_path, schema_v1.clone()).await.unwrap();
    ...
    assert_eq!(results[0].num_columns(), 4, "Should have 4 columns (unified V3 schema)");
    assert_eq!(results[0].num_rows(), 4, "Should have 4 rows");

    // Verify NULL handling across versions
    let age_array = results[0].column(2).as_any().downcast_ref::<Int32Array>().unwrap();
    assert!(age_array.is_null(0), "Alice (V1) should have NULL age");
    assert!(age_array.is_null(1), "Bob (V1) should have NULL age");
    assert_eq!(age_array.value(2), 35, "Charlie (V2) should have age");
    assert_eq!(age_array.value(3), 28, "Diana (V3) should have age");

    let dept_array = results[0].column(3).as_any().downcast_ref::<StringArray>().unwrap();
