...
async fn test_zorder_basic() -> Result<()> {
    ...
    let count = results[0]
        ...
        .unwrap()
        .value(0);
    assert_eq!(count, id_counter as i64, "All {} rows should be present", id_counter);
    
    // Verify we can still query with filters on Z-ORDERED columns
    let results = db.query("SELECT COUNT(*) as count FROM data WHERE category = 'A' AND region = 'US'").await?;
    let filtered_count = results[0]
        ...
        .unwrap()
        .value(0);
    assert_eq!(filtered_count, 5, "Should have 5 rows for category A, region US");
...
async fn test_zorder_after_optimize() -> Result<()> {
    ...
    for i in 0..30 {
        let batch = RecordBatch::try_new(
            ...
            vec![
                Arc::new(Int32Array::from(vec![i])) as ArrayRef,
                Arc::new(StringArray::from(vec![if i % 3 == 0 { "high" } else if i % 3 == 1 { "medium" } else { "low" }])) as ArrayRef,
    ...
    let results = db.query("SELECT COUNT(*) as count FROM data WHERE category = 'high'").await?;
