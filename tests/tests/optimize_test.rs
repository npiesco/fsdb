...
async fn test_optimize_compaction() -> Result<()> {
    ...
    assert!(files_before >= 10, "Should have created many small files (found {})", files_before);
    ...
    let count_before = results[0]
        ...
        .unwrap()
        .value(0);
    assert_eq!(count_before, 100, "Should have 100 rows before optimization");
    ...
    info!("  Parquet files AFTER optimize: {} (includes old files for time travel)", files_after);
    ...
    let count_after = results[0]
        ...
        .unwrap()
        .value(0);
    assert_eq!(count_after, 100, "Should still have 100 rows after optimization");
    
    // Verify specific data is intact
    let results = db.query("SELECT id, value FROM data WHERE id = 0 OR id = 50 OR id = 99 ORDER BY id").await?;
    ...
    info!("  Physical files: {} -> {} (new compacted file added, old files kept for time travel)",
        files_before, files_after
    );
    info!("  Logical active files in Delta Lake: 1 (old {} files marked as removed in log)", files_before);
...
async fn test_optimize_with_target_size() -> Result<()> {
    ...
    let files_after = count_parquet_files(db_path)?;
    let size_after = get_total_parquet_size(db_path)?;
    info!("  After: {} files, {} bytes (includes old files for time travel)", files_after, size_after);
