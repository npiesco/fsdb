...
async fn test_data_skipping_with_filter() {
    ...
    let batch1 = RecordBatch::try_new(
        ...
        vec![
            ...
            Arc::new(Int32Array::from((20..=29).cycle().take(100).collect::<Vec<i32>>())),
        ],
    )
    .unwrap();
    ...
    let batch2 = RecordBatch::try_new(
        ...
        vec![
            ...
            Arc::new(Int32Array::from((40..=49).cycle().take(100).collect::<Vec<i32>>())),
        ],
    )
    .unwrap();
    ...
    let batch3 = RecordBatch::try_new(
        ...
        vec![
            ...
            Arc::new(Int32Array::from((60..=69).cycle().take(100).collect::<Vec<i32>>())),
        ],
    )
    .unwrap();
    ...
    let results = db.query("SELECT name, age FROM data WHERE age > 55 ORDER BY age").await.unwrap();
    
    // Should only return data from batch 3 (ages 60-69)
    let total_rows: usize = results.iter().map(|b| b.num_rows()).sum();
    assert_eq!(total_rows, 100, "Should return 100 rows from age 60-69 range");
    
    // Verify correct age range
    let first_batch = &results[0];
    let ages = first_batch.column(1).as_any().downcast_ref::<Int32Array>().unwrap();
    ...
    println!("Data skipping stats: total_files={}, files_read={}, files_skipped={}", 
        skipping_stats.total_files, skipping_stats.files_read, skipping_stats.files_skipped);
    
    // Debug: List parquet files
    let parquet_count = std::fs::read_dir(&db_path).unwrap()
        .filter(|e| e.as_ref().unwrap().path().extension().and_then(|s| s.to_str()) == Some("parquet"))
        .count();
    println!("Parquet files found: {}", parquet_count);
    
    assert!(skipping_stats.total_files >= 3, "Should have at least 3 files, got {}", skipping_stats.total_files);
    assert!(skipping_stats.files_skipped >= 2, "Should skip at least 2 files (age 20-30 and 40-50 ranges)");
    assert_eq!(skipping_stats.files_read, 1, "Should only read 1 file (age 60-70 range)");
}
...
async fn test_data_skipping_with_range() {
    ...
    let results = db.query("SELECT COUNT(*) as count FROM data WHERE value >= 210 AND value <= 290").await.unwrap();
    
    assert_eq!(results.len(), 1);
    let count = results[0].column(0).as_any().downcast_ref::<Int64Array>().unwrap();
    assert_eq!(count.value(0), 81); // 290 - 210 + 1
    
    let skipping_stats = db.get_data_skipping_stats().await.unwrap();
    assert_eq!(skipping_stats.files_skipped, 2, "Should skip 2 files (ranges 0-100 and 400-500)");
    assert_eq!(skipping_stats.files_read, 1, "Should only read 1 file (range 200-300)");
}
...
async fn test_data_skipping_with_strings() {
    ...
    let results = db.query("SELECT category FROM data WHERE category >= 'W' ORDER BY category").await.unwrap();
    
    let total_rows: usize = results.iter().map(|b| b.num_rows()).sum();
    assert_eq!(total_rows, 100, "Should return 100 rows from Watermelon category");
...
async fn test_no_data_skipping_when_all_needed() {
    ...
    for i in 0..3 {
        ...
        let batch = RecordBatch::try_new(
            schema.clone(),
            vec![
                Arc::new(Int32Array::from((start_id..start_id+100).collect::<Vec<i32>>())),
                Arc::new(Int32Array::from((start_id..start_id+100).collect::<Vec<i32>>())),
            ],
        )
        .unwrap();
