...
fn create_test_batch(schema: SchemaRef, ids: Vec<i32>, names: Vec<&str>, values: Vec<i32>) -> RecordBatch {
...
async fn test_recovery_after_abrupt_shutdown() {
    ...
    // Step 1: Create database and write some committed data
    {
        ...
        let results = db.query("SELECT COUNT(*) as count FROM data").await.expect("Failed to query");
        let count_col = results[0].column(0).as_any().downcast_ref::<arrow::array::Int64Array>().unwrap();
        assert_eq!(count_col.value(0), 3, "Should have 3 records before shutdown");
    } // Database dropped here, simulating abrupt shutdown
    
    // Step 2: Reopen database and verify data persisted
    {
        let db = DatabaseOps::open(db_path).await.expect("Failed to reopen database");
        
        let results = db.query("SELECT COUNT(*) as count FROM data").await.expect("Failed to query after recovery");
        let count_col = results[0].column(0).as_any().downcast_ref::<arrow::array::Int64Array>().unwrap();
        assert_eq!(count_col.value(0), 3, "Should recover all 3 committed records");
        
        // Verify specific records
        let results = db.query("SELECT id, name, value FROM data ORDER BY id").await.expect("Failed to query records");
        let id_col = results[0].column(0).as_any().downcast_ref::<Int32Array>().unwrap();
        let name_col = results[0].column(1).as_any().downcast_ref::<StringArray>().unwrap();
        let value_col = results[0].column(2).as_any().downcast_ref::<Int32Array>().unwrap();
...
async fn test_recovery_with_multiple_restarts() {
    ...
    {
        let db = DatabaseOps::open(db_path).await.expect("Failed to reopen 1");
    ...
    {
        let db = DatabaseOps::open(db_path).await.expect("Failed to reopen 2");
    ...
    {
        let db = DatabaseOps::open(db_path).await.expect("Failed to reopen 3");
        let results = db.query("SELECT COUNT(*) as count FROM data").await.expect("Failed to query");
        let count_col = results[0].column(0).as_any().downcast_ref::<arrow::array::Int64Array>().unwrap();
        assert_eq!(count_col.value(0), 3, "Should have all 3 records after multiple restarts");
        
        // Verify we can still write after recovery
        let batch = create_test_batch(schema.clone(), vec![4], vec!["fourth"], vec![4]);
        db.insert(batch).await.expect("Failed to insert after recovery");
        
        let results = db.query("SELECT COUNT(*) as count FROM data").await.expect("Failed to query");
        let count_col = results[0].column(0).as_any().downcast_ref::<arrow::array::Int64Array>().unwrap();
        assert_eq!(count_col.value(0), 4, "Should have 4 records after post-recovery write");
    }
...
async fn test_recovery_with_deleted_rows() {
    ...
    // Create database and insert records
    {
        ...
        let batch1 = create_test_batch(schema.clone(), vec![1, 2], vec!["keep1", "keep2"], vec![1, 2]);
        db.insert(batch1).await.expect("Failed to insert batch1");
        ...
        db.delete_rows_where("name = 'delete_me'").await.expect("Failed to delete rows");
    }
    
    // Reopen and verify deletion persisted
    {
        let db = DatabaseOps::open(db_path).await.expect("Failed to reopen after deletion");
        
        let results = db.query("SELECT COUNT(*) as count FROM data").await.expect("Failed to query");
        let count_col = results[0].column(0).as_any().downcast_ref::<arrow::array::Int64Array>().unwrap();
        assert_eq!(count_col.value(0), 2, "Should have 2 records after deletion persisted");
        
        // Verify the right records remain
        let results = db.query("SELECT name FROM data ORDER BY id").await.expect("Failed to query names");
        let name_col = results[0].column(0).as_any().downcast_ref::<StringArray>().unwrap();
...
async fn test_wal_corruption_handling() {
    ...
    // Create database and write data
    {
        ...
        let batch = create_test_batch(schema.clone(), vec![1, 2, 3], vec!["r1", "r2", "r3"], vec![1, 2, 3]);
        db.insert(batch).await.expect("Failed to insert");
    }
    ...
    // Reopen database - Delta Lake transaction log provides durability
    {
        ...
        let results = db.query("SELECT COUNT(*) as count FROM data").await.expect("Failed to query");
        let count_col = results[0].column(0).as_any().downcast_ref::<arrow::array::Int64Array>().unwrap();
        assert_eq!(count_col.value(0), 3, "Should recover data from Delta Lake transaction log");
    }
...
async fn test_transaction_log_based_recovery() {
    ...
    assert!(fs::metadata(&delta_log_dir).is_ok(), "Delta Lake transaction log directory should exist");
    ...
    // Reopen and verify Delta Lake transaction log can reconstruct state
    {
        ...
        let results = db.query("SELECT COUNT(*) as count FROM data").await.expect("Failed to query");
        let count_col = results[0].column(0).as_any().downcast_ref::<arrow::array::Int64Array>().unwrap();
        assert_eq!(count_col.value(0), 3, "Should recover all 3 transactions from Delta Lake log");
