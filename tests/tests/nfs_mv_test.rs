...
async fn test_nfs_mv_rename_file() {
    ...
    let verify_result = tokio::time::timeout(std::time::Duration::from_secs(10), async {
        ...
        assert!(!original_exists, "Original file should not exist after rename");
        
        // Renamed file SHOULD exist
        let renamed_metadata = tokio::fs::metadata(&renamed_file).await?;
        assert!(renamed_metadata.is_file(), "Renamed file should exist and be a file");
        
        // Content should be preserved
        let content = tokio::fs::read_to_string(&renamed_file).await?;
        assert_eq!(content, "Test content for mv", "File content should be preserved after rename");
...
async fn test_nfs_mv_rename_directory() {
    ...
    assert!(exists_before, "Original directory should exist before rename");
    ...
    let verify_result = tokio::time::timeout(std::time::Duration::from_secs(10), async {
        ...
        assert!(new_metadata.is_dir(), "New directory should exist and be a directory");
