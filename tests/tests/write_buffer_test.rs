...
async fn test_write_buffer_batches_multiple_appends() {
    ...
    let count_array = results[0].column(0).as_any().downcast_ref::<arrow::array::Int64Array>().unwrap();
    ...
    println!("Transactions created: {} (10 appends)", transactions_created);
    assert!(transactions_created < 10, 
...
async fn test_write_buffer_auto_flush_on_size() {
    ...
    println!("Auto-flush created {} transactions for 1100 rows (~1.1MB)", transactions);
    assert!(transactions > 0, "Should have committed at least one transaction");
    assert!(transactions < 1100, "Should batch writes, not create transaction per row");
}
