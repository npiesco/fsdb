...
- **NFS Server**: Mount Delta Lake as POSIX filesystem - Unix commands (`cat`, `grep`, `echo >>`, `sed -i`) trigger Delta Lake transactions
- **Storage Backends**: Works with local filesystem (`/path/to/database`) and S3/MinIO (`s3://bucket/path`) - same unified API
